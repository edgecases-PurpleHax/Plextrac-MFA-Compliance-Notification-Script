...
def authenticate_and_send_email():
    ...
    Searches for users based on specific email domains from 'config.json'.
    ...
    def search_emails_in_data(json_obj):
        search_terms = config.get("customer_domains", [])  # Get customer domains from config
        matches = []
        ...
        if isinstance(json_obj, dict) and 'data' in json_obj and isinstance(json_obj['data'], list):
            for item in json_obj['data']:
                if isinstance(item, dict) and 'email' in item:
                    email = item['email']
                    if any(domain in email for domain in search_terms):
                        matches.append(item)
        return matches

    # Retrieve users from Plextrac API based on authenticated session
    user_endpoint = requests.get(url=f"{config['plextrac_url']}/api/v2/tenants/{r.json()['tenant_id']}/users",
    ...
    recipient_email = config['poc_email']  # Replace with the recipient's email address

    # Prepare the email subject
    subject = "MFA Non-compliant Users"

    # Construct the email body with user information
    body = f"The following users are not compliant with the {', '.join(config['customer_domains'])} requirement for MFA enabled on the Plextrac " \
           "Platform:\n\n"
